...
const TYPE_MAPPING = {
  ...
  // Telefoncu / telefon aksesuarcısı
  'telefoncu': 'shop=mobile_phone|shop=electronics|shop=computer',
}

// Serbest metin araması için ek anahtar kelimeler
const TYPE_KEYWORDS = {
  telefoncu: 'telefoncu iletişim gsm cep telefonu telefon aksesuarı cep iletişim iletişim merkezi',
}

export async function searchPlaces(type, location, filters = {}) {
  try {
    ...
    const typePart =
      businessName ||
      TYPE_KEYWORDS[baseType] ||
      type

    const locationPart = city || location

    let searchQuery = typePart
    if (district) searchQuery += ` ${district}`
    if (locationPart) searchQuery += ` ${locationPart}`
    searchQuery += ' Turkey'
